...
class GoogleTranslate
{
    ...
    public function translate(string $string): ?string
    {
        ...
        if($responseArray['code'] == 400){
            return serialize(['status'=>'Error','msg'=>$responseArray['msg'],'code'=>400,'ecode'=>$responseArray['ecode']]);
        }
        /*
        ...
        if (is_string($responseArray)) {
            if($responseArray == ''){
                return serialize(['status'=>'Error','msg'=>'We can not able to translate your string. May be invalid source language / target language / String or due to technical error','code'=>400]);
            }else if($responseArray == $string){
                return serialize(['status'=>'Error','msg'=>'We can not able to translate your string, so we return same string. May be source language or target language are same or not abe to this sorce language to this target language or technical error','translate'=>$responseArray,'dslang'=> $this->lastDetectedSource,'code'=>404]);
            }else{
                return serialize(['status'=>'Success','msg'=>'Translate successfully your string in target language','translate'=>$responseArray,'dslang'=> $this->lastDetectedSource,'code'=>200]);
            }
        } else {
            if (is_array($responseArray[0])) {
                $Result = (string) array_reduce($responseArray[0], function ($carry, $item) {
                    $carry .= $item[0];
                    return $carry;
                });
                if($Result == ''){
                    ...
                    return serialize(['status'=>'Error','msg'=>'We can not able to translate your string, so we return same string. May be source language or target language are same or not abe to this sorce language to this target language or technical error','translate'=>$Result,'dslang'=> $this->lastDetectedSource,'code'=>404]);
                }else{
                    return serialize(['status'=>'Success','msg'=>'Translate successfully your string in target language','translate'=>$Result,'dslang'=> $this->lastDetectedSource,'code'=>200]);
                }
            } else {
                if($responseArray[0] == ''){
                    return serialize(['status'=>'Error','msg'=>'We can not able to translate your string. May be invalid source language / target language / String or due to technical error','code'=>400]);
                }else if($responseArray[0] == $string){
                    return serialize(['status'=>'Error','msg'=>'We can not able to translate your string, so we return same string. May be source language or target language are same or not abe to this sorce language to this target language or technical error','translate'=>$responseArray[0],'dslang'=> $this->lastDetectedSource,'code'=>404]);
                }else{
                    return serialize(['status'=>'Success','msg'=>'Translate successfully your string in target language','translate'=>$responseArray[0],'dslang'=> $this->lastDetectedSource,'code'=>200]);
                }
            }
        }
    }
    ...
    public function getResponse(string $string): array
    {
        ...
        if ($connected){
            ...
            return ['status'=>'Error','msg'=>'Internet connection is not connected','code'=>400,'ecode'=>408];
