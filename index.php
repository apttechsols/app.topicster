<?php
	...
	define('PageSetting', ['BrowserTitle'=>'Topicster : Home','PageTitle'=>'Home','HeaderMenuActive'=>'Home','keywords'=>'Topicster, news, Article, sports, education, politics, programming','description'=>'Topicster provide articles on sports, politics, education, programming etc.','og_url'=>DomainName,'ThumbnailImage'=>[['Image'=>DomainName.'/Library/ImageStore/TopicsterLogo_Red.png','Width'=>$ThumbnailImageWidth,'Height'=>$ThumbnailImageHeight]],'og_type'=>'article','robots'=>'INDEX, FOLLOW']);
	...
	$GetArticle = AptPdoFetchWithAes(['Condtion'=> "Status::::Publish",'FetchData'=>'Category::::Url::::FUrl::::Title::::ModTitle::::PublishTime::::Description::::Author::::ImageUrl', 'DbCon'=> $PdoTopicsterPostData, 'TbName'=> 'post', 'EPass'=> $EPass,'FetchRowNo'=>'All','Limit'=>10,'DataOrder'=>'DESC|PublishTime']);
	if($GetArticle['code'] == 200){
		$ArticleDtls = array();
		$i=0;
		foreach ($GetArticle['msg'] as $key => $value) {
			$ArticleDtls[$i] = array();
			$ArticleDtls[$i]['Url'] = $value->Url;
			$ArticleDtls[$i]['FUrl'] = $value->FUrl;
			$ArticleDtls[$i]['Title'] = $value->Title;
			$ArticleDtls[$i]['ModTitle'] = $value->ModTitle;
			$ArticleDtls[$i]['PublishTime'] = $value->PublishTime;
			$ArticleDtls[$i]['Description'] = $value->Description;
			$ArticleDtls[$i]['ImageUrl'] = unserialize($value->ImageUrl);
			$GetAuthorDtls = AptPdoFetchWithAes(['Condtion'=> "Status::::Active::,::UserUrl::::".explode('_', $value->Author)[0].'_'.explode('_', $value->Author)[1],'FetchData'=>'FullName', 'DbCon'=> $PdoTopicsterPartner, 'TbName'=> explode('_', $value->Author)[0].'_'.explode('_', $value->Author)[0].'_account', 'EPass'=> $EPass]);
			if($GetAuthorDtls['code'] == 200){
				$ArticleDtls[$i]['Author'] = $GetAuthorDtls['msg']->FullName;
			}else{
				$ArticleDtls[$i]['Author'] = 'Unknown';
			}
			...
			if($GetPartnerDtls['code'] == 200){
				$ArticleDtls[$i]['AccountName'] = $GetPartnerDtls['msg']->AccountName;
			}else{
				unset($ArticleDtls[$i]); 
				continue;
			}
...
?>
<meta name="keywords" content="Topicster, latest news, latest article, breaking news, sport, politics, education, programming, movie, tv show" />
<meta name="news_keywords" content="Topicster, latest news, latest article, breaking news, sport, politics, education, programming, movie, tv show" />
<meta name="description" content="Here we provide articles on sport, politics, education, programming, movie, tv show etc. Topicster - The best way to keep updated" />
<style>
	.PostContent > p{
		color: #2f2f2f;
	    font-size: 16px;
		...
		<div style='margin-top: 25px;'>
			...
			foreach ($ArticleDtls as $key => $value) { ?>
				<div class="PostContent">
					<div><a href='Article/index.php?<?php echo $value['FUrl']; ?>' class='Title' id='<?php echo $value['Url']; ?>' name='<?php echo $value['Url']; ?>'><?php echo AptSafeTextDecode(['Str'=>$value['Title'],'EWords'=>AptSafeTextDecodeEWords])['msg']; ?></a></div>
					<div class="PostInfo">
						<span class="NameAndDate"><i class="fas fa-user"></i><?php echo $value['Author']; ?> at <?php echo date("d-M-Y",$value['PublishTime']); ?></span>&nbsp;&nbsp;&nbsp;&nbsp;
						<span class="views"><i class="fas fa-eye"></i>1000K @<?php echo $value['AccountName']; ?></span>
					</div>
					<div class="PostImages">
						<div class="MultiCarousel" data-items="1,2,3,4" data-slide="1" id="MultiCarousel"  data-interval="1000">
				            <div class="MultiCarousel-inner" id="MultiCarouselInner">
					...
					<div class='PostDescription'><p><?php echo strip_tags(AptSafeTextDecode(['Str'=>$value['Description'],'EWords'=>AptSafeTextDecodeEWords])['msg']); ?></p></div>
				</div>
			<?php }
